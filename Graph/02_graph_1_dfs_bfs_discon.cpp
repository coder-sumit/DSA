// dfs and bfs for disconnected graph
...
void graph_bfs(int **edges, int n, int sv, bool *visited)
{
    queue<int> q;
    q.push(sv);
    visited[sv] = true;
...
int main()
{
    ...
    //  calling on graph_dfs for disconnected graph too
    for (int i = 0; i < v; i++)
    {
        if (visited[i] == false)
        {
            graph_dfs(edges, v, i, visited);
        }
    }
